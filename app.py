...
# --- NUEVAS IMPORTACIONES PARA OPCIÓN 2 (PLOTLY) ---
...
def clean_reservation_df(df, tipo="puesto"):
    ...
    if tipo == "puesto":
        rename = {'user_name': 'Nombre', 'user_email': 'Correo', 'piso': 'Piso', 'reservation_date': 'Fecha Reserva', 'team_area': 'Ubicación'}
        df = df.rename(columns=rename)
        desired = ['Fecha Reserva', 'Piso', 'Ubicación', 'Nombre', 'Correo']
        existing = [c for c in desired if c in df.columns]
        return df[existing]
    elif tipo == "sala":
        rename = {'user_name': 'Nombre', 'user_email': 'Correo', 'piso': 'Piso', 'room_name': 'Sala', 'reservation_date': 'Fecha', 'start_time': 'Inicio', 'end_time': 'Fin'}
        df = df.rename(columns=rename)
        desired = ['Fecha', 'Inicio', 'Fin', 'Sala', 'Piso', 'Nombre', 'Correo']
        existing = [c for c in desired if c in df.columns]
        return df[existing]
    return df
...
def create_plotly_office_viz(p_sel, d_sel, zonas, df_d):
    ...
    st.subheader(f"🎨 Vista de Planta Interactiva - {p_sel}")
    ...
    if p_sel in zonas:
        for i, zona in enumerate(zonas[p_sel]):
            ...
            # Rectángulo de la zona (usando coordenadas tipo pixel)
    ...
    # Configurar layout para que parezca un plano
    fig.update_layout(
        xaxis=dict(visible=False, range=[0, 1000], autorange=False), # Rango fijo para simular canvas
        yaxis=dict(visible=False, range=[1000, 0], autorange=False), # Invertido para que (0,0) sea arriba-izquierda
        plot_bgcolor='white',
        paper_bgcolor='white',
    ...
    if p_sel in zonas:
        show_zone_stats(zonas[p_sel])

def show_zone_stats(zonas_piso):
    """Muestra estadísticas atractivas de las zonas"""
    st.divider()
    cols = st.columns(3)
    total_area = sum(zona['w'] * zona['h'] for zona in zonas_piso)
    teams = len(set(zona['team'] for zona in zonas_piso))
    
    with cols[0]: st.metric("Total de Zonas Definidas", len(zonas_piso))
    with cols[1]: st.metric("Equipos Asignados", teams)
    # Calcular ocupación aproximada del "canvas" de 1000x1000
    ocupacion = (total_area / (1000*1000)) * 100 
    with cols[2]: st.metric("Ocupación del Plano", f"{ocupacion:.1f}%")


# Función contenedora para el Editor Visual (Usa Plotly para ver, Sliders para editar)
def enhanced_zone_editor(p_sel, d_sel, zonas, df_d, global_logo_path):
    """Wrapper que usa la OPCIÓN 2 (Plotly) para visualizar y sliders para editar"""
    
    # 1. Mostrar la visualización interactiva (Plotly)
    create_plotly_office_viz(p_sel, d_sel, zonas, df_d)
    
    st.divider()
    st.subheader("🛠️ Editar Zonas (Modo Manual)")
    st.info("Usa los controles abajo para agregar nuevas zonas. El gráfico de arriba se actualizará al guardar.")
    
    # 2. Formulario de Edición Manual (Sliders fiables)
    with st.form(f"plotly_editor_form_{p_sel}"):
        c1, c2 = st.columns(2)
        ...
        # Agregar salas
        salas = ["Sala Reuniones Pequeña", "Sala Reuniones Grande"]
        eqs += salas
        
        with c1:
            equipo = st.selectbox("Equipo / Sala", eqs, key=f"plotly_team_{p_sel}")
            color = st.color_picker("Color", "#4ECDC4", key=f"plotly_color_{p_sel}")
            if equipo and equipo in current_seats_dict:
                 st.caption(f"Cupos asignados este día: {current_seats_dict[equipo]}")
        
        with c2:
            # Sliders mapeados al rango 0-1000 del gráfico Plotly
            x = st.slider("Posición X (Horizontal)", 0, 900, 100, key=f"plotly_x_{p_sel}")
            y = st.slider("Posición Y (Vertical)", 0, 900, 100, key=f"plotly_y_{p_sel}")
            w = st.slider("Ancho", 50, 500, 150, key=f"plotly_w_{p_sel}")
            h = st.slider("Alto", 50, 500, 100, key=f"plotly_h_{p_sel}")
        ...
        if st.form_submit_button("💾 Guardar Zona"):
            if equipo:
                ...
                save_zones(zonas)
                st.success("Zona agregada. El gráfico se actualizará.")
                st.rerun()
            else:
                st.error("Selecciona un equipo")

    # 3. Listado para borrar
    if p_sel in zonas and zonas[p_sel]:
        st.markdown("#### Zonas Activas")
        for i, z in enumerate(zonas[p_sel]):
            with st.container(border=True):
                c1, c2 = st.columns([4, 1])
                c1.markdown(f"**{z['team']}** (Pos: {z['x']},{z['y']} | Tam: {z['w']}x{z['h']})")
                if c2.button("🗑️", key=f"plotly_del_{p_sel}_{i}"):
                    zonas[p_sel].pop(i)
                    save_zones(zonas)
                    st.rerun()
...
if menu == "Vista pública":
    ...
    if df.empty: st.info("Sin datos.")
        ...
        with t2:
            ...
            # --- INTEGRACIÓN OPCIÓN 2 EN VISTA PÚBLICA ---
