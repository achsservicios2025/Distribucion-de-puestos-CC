...
def generate_full_pdf(distrib_df, semanal_df, out_path="reporte.pdf", logo_path=Path("static/logo.png"), deficit_data=None):
    ...
    try:
        # CORRECCIÓN: Normalización de nombres para evitar el error
        df_calc = distrib_df.copy()
        # Convertimos nombres de columnas a minúsculas y strip para buscar 'equipo' y '%distrib' o 'pct'
        ...
        if col_pct and col_eq:
            ...
            weekly_stats = df_calc.groupby(col_eq)[col_pct].mean().reset_index()
            ...
            pdf.cell(0, 6, clean_pdf_text("No se encontraron columnas de Equipo o Porcentaje para el resumen."), ln=True)
    ...
    if df.empty: st.info("Sin datos.")
        ...
        with t2:
            ...
            if ds == "Todos (Lunes a Viernes)":
                ...
                # --- Lógica agregada para forzar la generación antes de mostrar/descargar ---
                # 1. Obtener la distribución del día
                ...
                if not current_seats:
                    ...
                    # 2. Generar el plano (usa la configuración por defecto o la última guardada en admin)
...
# ==========================================
    ...
    # ---------------------------------------------------------
        ...
        sl = c_sala.selectbox("Selecciona Sala", salas_disponibles)
        
        # Lógica de detección de piso mejorada para nombres largos
        pi_s = "Piso 1" # Default
    ...
    # ---------------------------------------------------------
        ...
        if q:
            # Limpiar DataFrames para evitar KeyError
...
# ==========================================
    ...
    # -----------------------------------------------------------
    # T2: EDITOR VISUAL (CORREGIDO INDENTACIÓN Y LÓGICA)
    # -----------------------------------------------------------
    with t2:
        ...
        if pim.exists():
            # =========================================================
            # SOLUCIÓN DEFINITIVA ERROR 'str object has no attribute height'
            # Y PANTALLA BLANCA: Usamos PIL + Base64 Data URL
            # =========================================================
            ...
            if w_orig > cw:
                ...
                # Redimensionar usando PIL (Python)
            ...
            # 3. Convertir a Base64 (Esto evita que el Canvas falle internamente)
            buffered = BytesIO()
            ...
            canvas = st_canvas(
                ...
                stroke_width=2,
                stroke_color="#00A04A",
                background_image=img_data_url, # <-- Usamos la URL de datos
                update_streamlit=True,
                width=cw,
                height=ch,
                drawing_mode="rect",
                key=f"cv_{p_sel}_fixed_v2"
            )

            # 5. Controles de Guardado (Ahora fuera del bloque if img_str para evitar indent error)
            current_seats_dict = {}
            eqs = [""]
            if not df_d.empty:
            ...
            # Mostrar controles debajo del canvas
            st.markdown("---")
            cc1, cc2, cc3 = st.columns([2, 1, 1])
            tn = cc1.selectbox("Equipo / Sala", eqs)
            tc = cc2.color_picker("Color", "#00A04A")

            if tn and tn in current_seats_dict:
                st.info(f"Cupos asignados para {tn}: {current_seats_dict[tn]}")

            # Botón Guardar
            if cc3.button("Guardar Zona", key="save_zone_btn"):
                if tn and canvas.json_data and canvas.json_data.get("objects"):
                    ...
                    st.warning("Primero dibuja un recuadro en el plano y selecciona un equipo.")

            st.divider()

            # Listado y eliminación de zonas guardadas
            if p_sel in zonas:
                st.markdown("###### Zonas registradas en este piso:")
                for i, z in enumerate(zonas[p_sel]):
                    ...
                    if col_z2.button("X", key=f"del_zone_{i}"):
                        zonas[p_sel].pop(i)
                        save_zones(zonas)
                        st.rerun()
            ...
            with st.expander("🎨 Editar Estilos", expanded=True):
                ...
                ff_t = cf1.selectbox("Tipografía (Título)", ["Arial", "Calibri", "Comic Sans MS", "Courier New", "Georgia", "Impact", "Roboto", "Segoe UI", "Tahoma", "Times New Roman", "Trebuchet MS", "Verdana"], key="font_t")
                fs_t = cf2.selectbox("Tamaño Letra (Título)", [10, 12, 14, 16, 18, 20, 24, 28, 30, 32, 36, 40], index=5, key="size_t")
                align = cf3.selectbox("Alineación (Título)", align_options, index=1)

                # Estilos subtítulo
                cs1, cs2, cs3 = st.columns(3)
                ff_s = cs1.selectbox("Tipografía (Subtítulo)", ["Arial", "Calibri", "Comic Sans MS", "Courier New", "Georgia", "Impact", "Roboto", "Segoe UI", "Tahoma", "Times New Roman", "Trebuchet MS", "Verdana"], key="font_s")
                fs_s = cs2.selectbox("Tamaño Letra (Subtítulo)", [10, 12, 14, 16, 18, 20, 24, 28], index=3, key="size_s")
                align_s = cs3.selectbox("Alineación (Subtítulo)", align_options, index=1)

                # Estilos leyenda
                cl1, cl2, cl3 = st.columns(3)
                ff_l = cl1.selectbox("Tipografía (Leyenda)", ["Arial", "Calibri", "Comic Sans MS", "Courier New", "Georgia", "Impact", "Roboto", "Segoe UI", "Tahoma", "Times New Roman", "Trebuchet MS", "Verdana"], key="font_l")
                fs_l = cl2.selectbox("Tamaño Letra (Leyenda)", [8, 10, 12, 14, 16, 18], index=2, key="size_l")
    ...
    # -----------------------------------------------------------
    # T6: MANTENIMIENTO (OPCIONES RESTAURADAS)
    # -----------------------------------------------------------
    with t6:
        st.subheader("Mantenimiento")
        opt = st.radio("Selecciona qué borrar:", ["Reservas", "Distribución", "Planos/Zonas", "TODO"])
