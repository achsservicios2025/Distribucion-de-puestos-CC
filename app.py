...
if menu == "Vista pública":
    ...
    if df.empty: st.info("Sin datos.")
        ...
        with t1:
            ...
            st.dataframe(df_view, hide_index=True, width=None)
            
            st.subheader("Cupos libres por piso y día")
            st.dataframe(lib, hide_index=True, width=None)
...
# ==========================================
    ...
    # ---------------------------------------------------------
        ...
        with st.expander("Ver Listado General de Reservas", expanded=True):
            ...
            st.dataframe(clean_reservation_df(list_reservations_df(conn)), hide_index=True, width=None)

            st.markdown("<br>", unsafe_allow_html=True) 

            # TÍTULO CORREGIDO 2
            st.subheader("Reserva de salas") 
            st.dataframe(clean_reservation_df(get_room_reservations_df(conn), "sala"), hide_index=True, width=None)
